...
var Lunar = (function () {
  ...
  Lunar = class Lunar {
    static initClass() {
      ...
      this.prototype.state = PLAYING;

      this.prototype.msg = null;
      this.prototype.pressedKeys = {};

      this.prototype.levelIndex = 0;
      this.prototype.highScore = 0;

      this.prototype.hasFocus = false;
      this.prototype.hasListeners = false;
      this.prototype.hasLoop = false;

      this.prototype.miniRadius = 0;

      // game over animation...

      this.prototype.shoes = [];
    }
    ...
    crashed() {
      this.state = CRASHED;
      this.attempts++;
      // con.log("crashed attempts", @attempts)
      return setTimeout(() => this.restartLevel(), 1000);
    }

    restartLevel() {
      // con.log('restartLevel')
    ...
    nextLevel() {
      ...
      con.log("nextLevel", this.levelIndex);
    ...
    saveScore() {
      if (
        (typeof Airtasker !== "undefined" && Airtasker !== null
          ? Airtasker.currentUser
          : undefined) != null
      ) {
        const currentHighScore = Airtasker.currentUser.get(
          "web_display_options"
        ).lunar_highscore;
        ...
        con.log("Actually saving high score!");
        // return service.setDisplayOptions(
        //   { lunar_highscore: this.highScore },
        //   (response) => con.log("score saved", response),
        //   (response) => con.log("couldn't save score")
        // );
      }
      return con.log("no onelogged in");
    }
      ...
      switch (this.ship.state) {
        ...
        case this.ship.LANDED:
          if (this.ship.platform != null) {
            if (
              this.ship.platform.dropoff != null ||
              this.ship.platform.pickup != null
            ) {
              if (this.ship.platform.dropoff != null) {
                if (
                  this.ship.carrying != null &&
                  this.ship.carrying === this.ship.platform.dropoff
                ) {
                  this.msg = {
                    title: "Well done",
        ...
        case this.ship.ORBIT:
          ...
          return (this.msg = {
            ...
            summary: [
              "You flew too high.",
              "You are either in orbit or lost in space",
            ],
            state: "crashed",
          });
    ...
    render(msg) {
      ...
      this.offscreen.context.drawImage(
        ...
        this.ship.y - this.ship.h
      ); // ship's position is bottom centre of ship.
      ...
      if (this.ship.blasters.up) {
        this.offscreen.context.fillRect(
          this.ship.x - 10 - 2,
          this.ship.y + 1,
          4,
          this.num(1, 10)
      ...
      if (this.ship.blasters.right) {
        this.offscreen.context.fillRect(
          this.ship.x - this.ship.w / 2 + 15,
          this.ship.y - 10,
          -this.num(1, 10),
          3
        ); // left blaster ... moving right
      }
      ...
      if (this.ship.y > 0) {
        ...
        this.stage.context.lineTo(
          this.ship.x + this.miniRadius,
          50 + this.miniRadius
        );
        this.stage.context.lineTo(
          this.ship.x - this.miniRadius,
      ...
      if (this.level.platforms.length > 1) {
        for (const platform of Array.from(this.level.platforms)) {
          if (
            this.ship.carrying != null &&
            platform.dropoff != null &&
            platform.dropoff === this.ship.carrying
          ) {
            target = platform;
            break;
      ...
      this.stage.context.restore();
      return this.renderHUD(doRender, this.msg);
    }

    renderHUD(doRender, msg) {
      this.hud.render(doRender, this.ship, this.level, msg);
      return this.stage.context.drawImage(this.hud.canvas, 0, 0);
    }
    ...
    explodeShoes() {
      ...
      return (() => {
        ...
        for (let i = 0; i < this.shoes.length; i++) {
          ...
          if (
            s.x < -100 ||
            s.x > this.stage.w + 100 ||
            s.y < -100 ||
            s.y > this.stage.h + 100
          ) {
