...
export default class World extends Canvas {
	...
	createLand(level) {
		...
		for (x = 0, end = this.w, asc = end >= 0; asc ? x < end : x > end; asc ? x++ : x--) {
			// safeColour = null
		...
		let grd = this.sky.context.createLinearGradient(this.w * 0.25, 0, this.w * 0.75, this.h);
	...
	render() {
		...
		this.sun.x = (Math.sin(this.sun.day * -0.001) * this.w * 2) / 3 + (this.w * 3) / 4;
		this.sun.y = (Math.cos(this.sun.day * -0.0011) * this.h * 2) / 3 + (this.h * 1) / 3;
	...
	renderRain() {
		...
		return (() => {
			...
			for (let i = 0; i < this.rainDrops.length; i++) {
				...
				if (s.x < -100 || s.x > this.w + 100 || s.y < -100 || s.y > this.h + 100) {
