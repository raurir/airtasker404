...
export default class HUD extends Canvas {
  ...
  stat(index, txt, num) {
    ...
    if (num != null) {
      return this.drawText(this.context, this.x + 30, y, num);
    }
  }
  ...
  render(doRender, ship, level, msg) {
    ...
    if (doRender) {
      ...
      this.drawBlock(
        ...
        barX + 2,
        barY + 2,
        (ship.fuel / level.fuel) * (barWidth - 4),
        6,
        colourFuel
      );

      level.time.passed =
        Math.round((new Date().getTime() - level.time.start) / 100) / 10;
        ...
        return (() => {
          ...
          for (let i = 0; i < summary.length; i++) {
            ...
            if (s != null) {
              ...
              this.context.textAlign = "right";
              result.push(
                this.drawText(this.context, this.w / 2 + 150, y, s[2])
              );
            } else {
